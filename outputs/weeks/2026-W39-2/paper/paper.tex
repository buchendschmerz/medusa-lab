...
\lhead{\small\scshape A Preprint \textemdash{} Medusa Lab 2026-W39-2}
...
\begin{itemize}
  \item How does the structure of real (online) social networks change how fast new words become conventions?
  \item Do human groups follow the $N^{3/2}$ consensus-time scaling predicted for well-mixed naming games?
  \item How much lexical memory do speakers need during the negotiation phase?
\end{itemize}
...
Literature retrieval was unavailable in this run, so the discussion relies on the foundational references of the model.
...
The simulation (coder/sim.py) was executed in Medusa Lab's sandbox (isolation: strict); the full run took 2.0 seconds. The code, parameters and raw data tables are archived with this paper, so every number and figure can be regenerated.
...
Medusa Lab repository (cycle 2026-W39-2). The run used seed 20260901, took 2.0\,s and was
